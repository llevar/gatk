...
run_id=0
for exec_cores in 8; do
  for total_cores in 32; do
    ...
    #time_gatk "BQSRPipelineSpark -I hdfs:///user/$USER/q4_spark_eval/out/markdups-sharded -O hdfs:///user/$USER/q4_spark_eval/out/bqsr-sharded --shardedOutput true -R hdfs:///user/$USER/q4_spark_eval/human_g1k_v37.2bit --knownSites hdfs:///user/$USER/q4_spark_eval/dbsnp_138.b37.vcf --joinStrategy OVERLAPS_PARTITIONER" $num_exec $exec_cores 32g 16g >> $RESULTS_CSV
    time_gatk "BQSRPipelineSpark -I bam/CEUTrio.HiSeq.WGS.b37.ch1.1m-65m.NA12878.bam -O hdfs:///user/$USER/q4_spark_eval/out/bqsr-sharded --shardedOutput true -R hdfs:///user/$USER/q4_spark_eval/human_g1k_v37.2bit --knownSites hdfs:///user/$USER/q4_spark_eval/dbsnp_138.b37.vcf --joinStrategy OVERLAPS_PARTITIONER" $num_exec $exec_cores 32g 16g >> $RESULTS_CSV
